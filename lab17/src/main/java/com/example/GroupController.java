...
public class GroupController {
    ...
    public ResponseEntity<?> filterGroupsByName(@RequestParam("groupName") String groupName) {
        try (Session session = sessionFactory.openSession()) {
            ...
            cq.where(cb.equal(root.get("groupName"), groupName));

            List<Group> groups = session.createQuery(cq).getResultList();

            return ResponseEntity.ok(groups);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error filtering groups by name.");
        }
    }
